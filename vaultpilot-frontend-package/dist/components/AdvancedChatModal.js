...
export class AdvancedChatModal extends Modal {
    ...
    createConversationSidebar(container) {
        // Header
    ...
    createContextSidebar(container) {
        // Context panel
    ...
    createMultiModalInput(container) {
    ...
    async loadAgents() {
        try {
            ...
            if (response.success && response.data) {
                ...
                this.availableAgents = Array.isArray(data) ? data : data.agents || [];
    ...
    onAgentChange() {
        ...
        const agentInfo = this.agentSelectorEl.nextElementSibling;
        if (selectedAgentId) {
          ...
          <div class="agent-details">
            ...
            <div class="agent-capabilities">
              ${((_a = agent.capabilities) === null || _a === void 0 ? void 0 : _a.map(cap => `<span class="capability-tag">${cap}</span>`).join('')) || ''}
            </div>
          </div>
        `;
            }
    ...
    updateEvolutionStatus() {
        var _a;
        const evolutionList = (_a = this.conversationBranchesEl.parentElement) === null || _a === void 0 ? void 0 : _a.querySelector('.evolution-list');
        if (evolutionList) {
    ...
    async updateSuggestions() {
        ...
        const suggestions = this.inputContainer.querySelector('.input-suggestions');
        if (input.length > 3) {
    ...
    applySuggestion(suggestion) {
        ...
        this.inputEl.focus();
        const suggestionsEl = this.inputContainer.querySelector('.input-suggestions');
    ...
    async sendMessage() {
        ...
        if (!message && this.multiModalAssets.length === 0)
            return;
        // Add user message
        this.addMessage({
            role: 'user',
    ...
    async streamResponse(request, messageEl) {
    ...
    async simulateStreaming(fullResponse, messageEl) {
    ...
    async getVaultContext() {
        ...
        if (activeView === null || activeView === void 0 ? void 0 : activeView.file) {
    ...
    updateMultiModalDisplay() {
        const assetsList = this.multiModalInputEl.querySelector('.attached-assets');
